...
mongoose.connect("mongodb://localhost/authentication" || db);
...
router.get("/open/course/keywords/:id", (req, res) => {
  ...
  try {
    data.filter((data) => {
      let newString = String(data.catalog_nbr)
        .concat(String(data.className))
        .replace(/\s/g, "");
  ...
  let results = stringSimilarity.findBestMatch(
    String(searchData).toUpperCase(),
    keywords
  );
...
router.get("/open/allReviews", (req, res) => {
  Review.find(
    { hidden: false, infringing: false },
    "title courseId rating comment createdBy createdAt",
    function (err, review) {
      if (err) {
...
router.get("/open/publicSchedules", (req, res) => {
  Schedule.find({ visibility: "public" })
...
router.get("/secure/user-detail", checkToken, (req, res, next) => {
  User.findOne({ email: req.email }, (err, user) => {
    if (!user) {
      ...
      return res
        .status(200)
        .json({ status: true, user: _.pick(user, ["name", "username"]) });
  });
});
...
router.put("/secure/user/:username", checkToken, function (req, res, next) {
  ...
  if (!req.body.admin) {
    res
      .status(400)
      .send("Invalid! Choose true or false to let user be admin or not.");
  }

  if (!req.body.deactive) {
    res
      .status(400)
      .send("Invalid! Choose true or false to deactivate user account!");
  } else {
...
router.get("/secure/reviews", (req, res) => {
  Review.find(
    {},
    "title comment rating courseId hidden createdBy",
    function (err, review) {
      if (err) {
        return console.error(err);
...
router.put("/secure/review/:title", checkToken, function (req, res, next) {
  if (!req.body.hidden) {
    res
      .status(400)
      .send(
        "Invalid! Choose true if you want to hide review or false if you want to show review."
      );
  } else {
    Review.findOneAndUpdate({ title: req.params.title }, req.body).then(
      function () {
...
router.get("/secure/schedule", (req, res) => {
  Schedule.find(
    ...
    "scheduleName scheduleDescription subject_schedule courseNumber_schedule",
...
router.get("/secure/schedule/:sched_name", (req, res) => {
  Schedule.findOne(
    { scheduleName: req.params.sched_name },
    function (err, schedule) {
      if (err) {
        console.error(err);
        res
          .status(400)
          .send(`Schedule ${req.params.sched_name} was not found!`);
      } else {
        res.send(JSON.stringify(schedule));
      }
    }
  ...
  function (req, res, next) {
    ...
    if (matchingSubjectResult == false) {
      res
        .status(400)
        .send(
          "Invalid Subject Entered. This subject is not offered at Western University"
        );
    }
    ...
    if (checkString() == false) {
      ...
      Schedule.findOneAndUpdate(
        ...
        Schedule.findOne({ scheduleName: req.params.sched_name }).then(
          function (schedule) {
            res.send(schedule);
          }
        );
...
router.delete("/secure/schedule/:sched_name", checkToken, (req, res, next) => {
  Schedule.findOneAndDelete({ scheduleName: req.params.sched_name }).then(
    function (schedule) {
      res.send(schedule);
    }
  );
...
router.get("/verify/:id", (req, res) => {
  User.findOne(
    { authenticationCode: req.params.id },
    function (err, foundObject) {
      if (foundObject != null) {
        ...
        foundObject.save(function (err, updatedObject) {
          ...
          if (updatedObject) {
            return res.end(
              "<h1>Your Email is now verified and you may login!</h1>"
            );
          }
        });
      } else {
